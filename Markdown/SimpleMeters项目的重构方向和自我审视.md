...
我第一次接触到std::optional<T>、std::unique_ptr<T>这类管理生命周期的标准库工具，是在写下这段文字前的两个月，不得不说我这几个月学习的东西太多了。但是即使我建议新手别上来就写这种大型的项目，但是你还是可以在各种错误中学习和认识到内存安全有关的问题，比如说所有权问题，前提是有一点——你的项目写完后不管，写完就扔。虽然我一开始就没想过写这个项目这么久，不然之后需要偿还的技术债是不得不还的，这就包括了刚刚说到的架构问题。如果我一开始就掌握内存安全和线程安全的知识，我是不是就能在选择架构时头脑能更清晰，如果一开始就掌握内存安全知识，在使用 std::shared_ptr 时就可能会多问一句：是不是当前架构或实现本身就有问题？同样，当你去修改，增加，删除代码中某些功能时需要思考很久，这是不是也是说明当前代码架构或实现有问题？这是我写这段文字时想到的一个问题
